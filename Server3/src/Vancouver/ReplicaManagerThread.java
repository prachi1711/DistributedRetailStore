...
public class ReplicaManagerThread extends Thread {
	 ...
	 protected String vancouverServer1Name = "";
	 ...
	 public ReplicaManagerThread(DatagramSocket rmSocket,DatagramPacket rcvPacket,byte[] rcvData,String vServer1Name, boolean flag)
	 {
		 this.rmSock = rmSocket;
		 this.receivePacketFromRM = rcvPacket;
		 this.receiveData = rcvData;
		 this.vancouverServer1Name = vServer1Name;
		 this.isNew = flag;
	 }
    public void run()
    {       
       if(isNew)
		...
		{					
			try
			{
			      ...
			      InetAddress IPAddress = InetAddress.getByName(vancouverServer1Name);
			      ...
			      IPAddress = InetAddress.getByName(vancouverServer1Name);
       ...
       VancouverServerImpl m = new VancouverServerImpl(itemHT,semaphoreHM);
       System.out.println("Vancouver Server 3 ready...");	
       int multiCastPortNo = 4448;
       InetAddress multiCastAddress = null;
       try 
       {
    	   multiCastAddress = InetAddress.getByName("228.5.6.5");
       } 
       catch (UnknownHostException e1) 
       {		
    	   e1.printStackTrace();
       }
       mcThread = new MultiCastThread(3002,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(6003); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
