...
public class VancouverServerImpl implements RetailStoreOperations{
									...
									if(islockedT && islockedM)							
									{
						             ...
						             if(totalItemsAvailable >= numberOfItem)
						             {
						            	 ...
						            	 else
						            	 {
						            		 ...
						            		 orderItemFromOtherStore(itemID,itemsAvailableWithTServer,6002);
						            		 orderItemFromOtherStore(itemID,tempNo,6001);
						            	 }
						            	 buyStatus = true;
						             }
						             else
	...
	return msg;
															...
															if(islockedT && islockedM)							
															{
												             ...
												             if(totalItemsAvailable >= desiredNumber)
												             {
												            	 ...
												            	 else
												            	 {
												            		 ...
												            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithTServer,6002);
												            		 orderItemFromOtherStore(desiredItemID,tempNo,6001);
												            	 }
												            	 buyStatus = true;
												             }
												             else
		...
		public Hashtable<String ,HashMap<Integer,Integer>> checkStockFromOtherServers(int itemID,Hashtable<String ,HashMap<Integer,Integer>> serverItemMap) 
		{
			...
			try
			{
			      ...
			      sendData = sendVal.getBytes();
			      sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 6001);
	 ...
	 */
		...
		public static void main(String[] args) 
		{
			try
		    {  						
				DatagramSocket rmSocket = new DatagramSocket(6007); // create a socket to bind with replica Manager using UDP
				byte[] receiveData = new byte[1024];	
