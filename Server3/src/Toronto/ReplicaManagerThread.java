...
public class ReplicaManagerThread extends Thread {
    ...
    {       
       ...
       System.out.println("Toronto Server 3 ready...");	
       ...
       mcThread = new MultiCastThread(2002,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(6002); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
