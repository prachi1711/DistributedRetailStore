...
public class TorontoServerImpl implements RetailStoreOperations {
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(semaphoreHM.containsKey(itemID))
					{
						...
						try
						{
							if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
							{
							   ...
							   try
							   {
									...
									{
										islockedM = intimateOtherServerRegLock(itemID,0,6001) ;
										islockedV = intimateOtherServerRegLock(itemID,0,6003);
										if(islockedM && islockedV)
										{
											...
											serverItemMap = checkStockFromOtherServers(itemID,serverItemMap);
								             ...
								             if(totalItemsAvailable >= noOfReqItem)
								             {
								            	 ...
								            	 {
								            		 orderItemFromOtherStore(itemID,noOfReqItem,6001);
								            	 }
								            	 else
								            	 {
								            		 noOfReqItem = noOfReqItem - itemsAvailableWithMServer;
								            		 orderItemFromOtherStore(itemID,itemsAvailableWithMServer,6001);
								            		 orderItemFromOtherStore(itemID,noOfReqItem,6003);
								            	 }
								            	 buyStatus = true;
									         }
								             else
									...
									else if(flag == 3) // item not available in the current store
									{
										...
										if(islockedM && islockedV)							
										{
							             ...
							             if(totalItemsAvailable >= numberOfItem)
							             {
							            	 ...
							            	 {
							            		 orderItemFromOtherStore(itemID,numberOfItem,6001);
							            	 }
							            	 else
							            	 {
							            		 int tempNo = 0;
							            		 ...
							            		 orderItemFromOtherStore(itemID,itemsAvailableWithMServer,6001);
							            		 orderItemFromOtherStore(itemID,tempNo,6003);
							            	 }
							            	 buyStatus = true;
							             }
							             else
						...
						finally
						{
							...
							if(islockedM)
							{
								intimateOtherServerRegLock(itemID, 1, 6001);	
							}
							if(islockedV)
							{
								intimateOtherServerRegLock(itemID, 1, 6003);
							}
						}
					}
					else
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(inputFile.exists())
					{
								    			...
								    			try
												   {
														...
														{
															islockedM = intimateOtherServerRegLock(desiredItemID,0,6001) ;
															islockedV = intimateOtherServerRegLock(desiredItemID,0,6003);
															if(islockedM && islockedV)							
															{
																Hashtable<String ,HashMap<Integer,Integer>> serverItemMap = new Hashtable<String ,HashMap<Integer,Integer>>();;
																...
																serverItemMap = checkStockFromOtherServers(desiredItemID,serverItemMap);
													             ...
													             if(totalItemsAvailable >= noOfReqItem)
													             {
													            	 ...
													            	 if(itemsAvailableWithMServer >= noOfReqItem)
													            	 {
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,6001);
													            	 }
													            	 else
													            	 {
													            		 noOfReqItem = noOfReqItem - itemsAvailableWithMServer;
													            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithMServer,6001);												            				 												            			
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,6003);
													            	 }
													            	 buyStatus = true;
														         }
													             else
														...
														else if(flag == 3) // item not available in the current store
														{
															...
															if(islockedM && islockedV)							
															{
												             ...
												             if(totalItemsAvailable >= desiredNumber)
												             {
												            	 ...
												            	 {
												            		 orderItemFromOtherStore(desiredItemID,desiredNumber,6001);
												            	 }
												            	 else
												            	 {
												            		 int tempNo = 0;
												            		 ...
												            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithMServer,6001);
												            		 orderItemFromOtherStore(desiredItemID,tempNo,6003);
												            	 }
												            	 buyStatus = true;
												             }
												             else
						...
						finally
						{
							...
							if(islockedM)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 6001);	
							}
							if(islockedV)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 6003);
							}
						}
		 ...
		 */
			...
			public Hashtable<String ,HashMap<Integer,Integer>> checkStockFromOtherServers(int itemID,Hashtable<String ,HashMap<Integer,Integer>> serverItemMap) 
			{
				...
				try
				{
				      ...
				      sendData = sendVal.getBytes();
				      sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 6003);
	...
	public static void main(String[] args) 
	{
		try
	    {  						
			DatagramSocket rmSocket = new DatagramSocket(6006); // create a socket to bind with replica Manager using UDP
			byte[] receiveData = new byte[1024];	
