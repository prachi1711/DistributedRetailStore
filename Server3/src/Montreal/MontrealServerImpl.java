...
public class MontrealServerImpl implements RetailStoreOperations{
		...
		/**
		 * Constructor for MontrealServerImpl 
		 */
		public MontrealServerImpl(HashMap<Integer, Integer> itemHT,HashMap<Integer, Semaphore> semaphoreHM ) {
			this.itemHT = itemHT;
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(semaphoreHM.containsKey(itemID))
					{
						...
						try
						{
							if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
							{
							   ...
							   try
							   {
									...
									{
										islockedT = intimateOtherServerRegLock(itemID,0,6002) ;
										islockedV = intimateOtherServerRegLock(itemID,0,6003);
										if(islockedT && islockedV)							
										{
											...
											serverItemMap = checkStockFromOtherServers(itemID,serverItemMap);
								             ...
								             totalItemsAvailable = itemsAvailableWithTServer + itemsAvailableWithVServer;
								             if(totalItemsAvailable >= noOfReqItem)
								             {
								            	 itemHT.remove(itemID);
								            	 if(itemsAvailableWithTServer >= noOfReqItem)
								            	 {
								            		 orderItemFromOtherStore(itemID,noOfReqItem,6002);
								            	 }
								            	 else
								            	 {
								            		 noOfReqItem = noOfReqItem - itemsAvailableWithTServer;
								            		 orderItemFromOtherStore(itemID,itemsAvailableWithTServer,6002);
										...
										{
											msg = "Could not acquire lock for item: "+itemID;
											buyStatus = false;
										}														
									}
									else if(flag == 3) // item not available in the current store
									{
										...
										if(islockedT && islockedV)							
										{
							             ...
							             if(totalItemsAvailable >= numberOfItem)
							             {
							            	 ...
							            	 {
							            		 orderItemFromOtherStore(itemID,numberOfItem,6002);
							            	 }
							            	 else
							            	 {
							            		 int tempNo = 0;
						...
						finally
						{
							...
							if(islockedT)
							{
								intimateOtherServerRegLock(itemID, 1, 6002);	
							}
							if(islockedV)
							{
	...
	return msg;
		...
		{
			// first check for if the number of items are valid for return
			String msg = "";
			File inputFile = null;
			FileReader fileReader = null;          
			int flag = 0;
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(inputFile.exists())
					{
								    			...
								    			try
												   {
														...
														{
															islockedT = intimateOtherServerRegLock(desiredItemID,0,6002) ;
															...
															if(islockedT && islockedV)							
															{
																...
																serverItemMap = checkStockFromOtherServers(desiredItemID,serverItemMap);
													             ...
													             if(totalItemsAvailable >= noOfReqItem)
													             {
													            	 ...
													            	 if(itemsAvailableWithTServer >= noOfReqItem)
													            	 {
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,6002);
													            	 }
													            	 else
													            	 {
													            		 noOfReqItem = noOfReqItem - itemsAvailableWithTServer;
													            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithTServer,6002);
														...
														else if(flag == 3) // item not available in the current store
														{
															...
															if(islockedT && islockedV)							
															{
												             ...
												             if(totalItemsAvailable >= desiredNumber)
												             {
												            	 ...
												            	 {
												            		 orderItemFromOtherStore(desiredItemID,desiredNumber,6002);
												            	 }
												            	 else
												            	 {
												            		 int tempNo = 0;
						...
						finally
						{
							...
							if(islockedT)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 6002);	
							}
							if(islockedV)
							{
					...
					try
					{
						if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
						{
						   ...
						   try
						   {
								...
								serverItemMap.put("Montreal", hItemMap);						
						   }
						   catch(Exception e)
						   {
							   e.printStackTrace();
		...
		public Hashtable<String ,HashMap<Integer,Integer>> checkStockFromOtherServers(int itemID,Hashtable<String ,HashMap<Integer,Integer>> serverItemMap) 
		{
			...
			try
			{
			      ...
			      sendData = sendVal.getBytes();
			      DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 6002);
	...
	public static void main(String[] args) 
	{
		try
	    {  						
			DatagramSocket rmSocket = new DatagramSocket(6005); // create a socket to bind with replica Manager using UDP
			byte[] receiveData = new byte[1024];	
