...
public class MontrealServerImpl implements RetailStoreOperations{
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(semaphoreHM.containsKey(itemID))
					{
						...
						try
						{
							if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
							{
							   ...
							   try
							   {
									...
									{
										islockedT = intimateOtherServerRegLock(itemID,0,4002) ;
										...
										if(islockedT && islockedV)							
										{
											...
											serverItemMap = checkStockFromOtherServers(itemID,serverItemMap);
								             ...
								             if(totalItemsAvailable >= noOfReqItem)
								             {
								            	 ...
								            	 {
								            		 orderItemFromOtherStore(itemID,noOfReqItem,4002);
								            	 }
								            	 else
								            	 {
								            		 noOfReqItem = noOfReqItem - itemsAvailableWithTServer;
								            		 orderItemFromOtherStore(itemID,itemsAvailableWithTServer,4002);
									...
									else if(flag == 3) // item not available in the current store
									{
										...
										if(islockedT && islockedV)							
										{
							             ...
							             if(totalItemsAvailable >= numberOfItem)
							             {
							            	 ...
							            	 {
							            		 orderItemFromOtherStore(itemID,numberOfItem,4002);
							            	 }
							            	 else
							            	 {
							            		 int tempNo = 0;
						...
						finally
						{
							...
							if(islockedT)
							{
								intimateOtherServerRegLock(itemID, 1, 4002);	
							}
							if(islockedV)
							{
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(inputFile.exists())
					{
								    			...
								    			try
												   {
														...
														{
															islockedT = intimateOtherServerRegLock(desiredItemID,0,4002) ;
															...
															if(islockedT && islockedV)							
															{
																...
																serverItemMap = checkStockFromOtherServers(desiredItemID,serverItemMap);
													             ...
													             if(totalItemsAvailable >= noOfReqItem)
													             {
													            	 ...
													            	 if(itemsAvailableWithTServer >= noOfReqItem)
													            	 {
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,4002);
													            	 }
													            	 else
													            	 {
													            		 noOfReqItem = noOfReqItem - itemsAvailableWithTServer;
													            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithTServer,4002);
														...
														else if(flag == 3) // item not available in the current store
														{
															...
															if(islockedT && islockedV)							
															{
												             ...
												             if(totalItemsAvailable >= desiredNumber)
												             {
												            	 ...
												            	 {
												            		 orderItemFromOtherStore(desiredItemID,desiredNumber,4002);
												            	 }
												            	 else
												            	 {
												            		 int tempNo = 0;
						...
						finally
						{
							...
							if(islockedT)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 4002);	
							}
							if(islockedV)
							{
	...
	public static void main(String[] args) 
	{
		try
	    {  						
			DatagramSocket rmSocket = new DatagramSocket(4005); // create a socket to bind with replica Manager using UDP
			byte[] receiveData = new byte[1024];	
