...
public class ReplicaManagerThread extends Thread {
	 ...
	 protected String montrealServer2Name = "";
	 ...
	 public ReplicaManagerThread(DatagramSocket rmSocket,DatagramPacket rcvPacket,byte[] rcvData,String mServer2Name, boolean flag)
	 {
		 this.rmSock = rmSocket;
		 this.receivePacketFromRM = rcvPacket;
		 this.receiveData = rcvData;
		 this.montrealServer2Name = mServer2Name;
		 this.isNew = flag;
	 }
    public void run()
    {       
       if(isNew)
		...
		{					
			try
			{
			      ...
			      InetAddress IPAddress = InetAddress.getByName(montrealServer2Name);
			      ...
			      IPAddress = InetAddress.getByName(montrealServer2Name);
       ...
       System.out.println("Montreal Server 1 ready...");	
       ...
       mcThread = new MultiCastThread(1000,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(4001); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
