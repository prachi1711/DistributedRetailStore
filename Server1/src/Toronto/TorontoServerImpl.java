...
public class TorontoServerImpl implements RetailStoreOperations {
					
		private static Logger logger = Logger.getLogger("TorontoServerImpl");	
		...
		/**
		 * Constructor for TorontoServerImpl 
		 */
		public TorontoServerImpl(HashMap<Integer, Integer> itemHT,HashMap<Integer, Semaphore> semaphoreHM ) {
			this.itemHT = itemHT;
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(semaphoreHM.containsKey(itemID))
					{
						...
						try
						{
							if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
							{
							   ...
							   try
							   {
									...
									{
										islockedM = intimateOtherServerRegLock(itemID,0,4001) ;
										islockedV = intimateOtherServerRegLock(itemID,0,4003);
										if(islockedM && islockedV)
										{
											...
											int itemsAvailableWithVServer = 0;
											...
											serverItemMap = checkStockFromOtherServers(itemID,serverItemMap);
								             ...
								             for (Enumeration e = serverItemMap.keys(); e.hasMoreElements();) 
								             {
								                 ...
								                 else if(key.equals("Vancouver"))
								                 {
								                	 for(Integer keys : serverItemMap.get(key).keySet())
									                 {
									                	 
								                		 itemsAvailableWithVServer = itemsAvailableWithVServer + serverItemMap.get(key).get(keys) ;
									                 } 
								                 }
								             }
								             totalItemsAvailable = itemsAvailableWithMServer + itemsAvailableWithVServer;
								             if(totalItemsAvailable >= noOfReqItem)
								             {
								            	 itemHT.remove(itemID);
								            	 if(itemsAvailableWithMServer >= noOfReqItem)
								            	 {
								            		 orderItemFromOtherStore(itemID,noOfReqItem,4001);
								            	 }
								            	 else
								            	 {
								            		 noOfReqItem = noOfReqItem - itemsAvailableWithMServer;
								            		 orderItemFromOtherStore(itemID,itemsAvailableWithMServer,4001);
								            		 orderItemFromOtherStore(itemID,noOfReqItem,4003);
								            	 }
								            	 buyStatus = true;
									         }
								             else
									...
									else if(flag == 3) // item not available in the current store
									{
										...
										if(islockedM && islockedV)							
										{
							             ...
							             {
							            	 itemHT.remove(itemID);
							            	 if(itemsAvailableWithMServer >= numberOfItem)
							            	 {
							            		 orderItemFromOtherStore(itemID,numberOfItem,4001);
							            	 }
							            	 else
							            	 {
							            		 int tempNo = 0;
							            		 tempNo = numberOfItem - itemsAvailableWithMServer;
							            		 orderItemFromOtherStore(itemID,itemsAvailableWithMServer,4001);
							            		 orderItemFromOtherStore(itemID,tempNo,4003);
							            	 }
							            	 buyStatus = true;
							             }
							             else
						...
						finally
						{
							...
							if(islockedM)
							{
								intimateOtherServerRegLock(itemID, 1, 4001);	
							}
							if(islockedV)
							{
								intimateOtherServerRegLock(itemID, 1, 4003);
							}
						}
					}
					else
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(inputFile.exists())
					{
								    			...
								    			try
												   {
														...
														{
															islockedM = intimateOtherServerRegLock(desiredItemID,0,4001) ;
															islockedV = intimateOtherServerRegLock(desiredItemID,0,4003);
															if(islockedM && islockedV)							
															{
																Hashtable<String ,HashMap<Integer,Integer>> serverItemMap = new Hashtable<String ,HashMap<Integer,Integer>>();;
																...
																serverItemMap = checkStockFromOtherServers(desiredItemID,serverItemMap);
													             ...
													             if(totalItemsAvailable >= noOfReqItem)
													             {
													            	 ...
													            	 if(itemsAvailableWithMServer >= noOfReqItem)
													            	 {
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,4001);
													            	 }
													            	 else
													            	 {
													            		 noOfReqItem = noOfReqItem - itemsAvailableWithMServer;
													            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithMServer,4001);												            				 												            			
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,4003);
													            	 }
													            	 buyStatus = true;
														         }
													             else
														...
														else if(flag == 3) // item not available in the current store
														{
															...
															if(islockedM && islockedV)							
															{
												             ...
												             {
												            	 itemHT.remove(desiredItemID);
												            	 if(itemsAvailableWithMServer >= desiredNumber)
												            	 {
												            		 orderItemFromOtherStore(desiredItemID,desiredNumber,4001);
												            	 }
												            	 else
												            	 {
												            		 int tempNo = 0;
												            		 tempNo = desiredNumber - itemsAvailableWithMServer;
												            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithMServer,4001);
												            		 orderItemFromOtherStore(desiredItemID,tempNo,4003);
												            	 }
												            	 buyStatus = true;
												             }
												             else
						...
						finally
						{
							...
							if(islockedM)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 4001);	
							}
							if(islockedV)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 4003);
							}
						}
					...
					try
					{
						if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
						{
						   ...
						   try
						   {
								...
								serverItemMap.put("Toronto", hItemMap);						
						   }
						   catch(Exception e)
						   {
							   e.printStackTrace();
		 ...
		 */
			...
			public Hashtable<String ,HashMap<Integer,Integer>> checkStockFromOtherServers(int itemID,Hashtable<String ,HashMap<Integer,Integer>> serverItemMap) 
			{
				...
				try
				{
				      ...
				      sendData = sendVal.getBytes();
				      sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 4003);
				      ...
				      itemList = new HashMap<Integer,Integer>();
				      itemList.put(itemID, Integer.parseInt(returnVal));
				      serverItemMap.put("Vancouver",itemList);
				}
				catch(Exception e)
				{
					e.printStackTrace();
	...
	public static void main(String[] args) 
	{
		try
	    {  						
			DatagramSocket rmSocket = new DatagramSocket(4006); // create a socket to bind with replica Manager using UDP
			byte[] receiveData = new byte[1024];	
			DatagramPacket receivePacket = null;			
	        ReplicaManagerThread rmThread = new ReplicaManagerThread(rmSocket,receivePacket,receiveData,args[0],true); // arg[0] has Toronto 2 host name
	        rmThread.start();		        
