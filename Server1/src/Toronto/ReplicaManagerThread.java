...
public class ReplicaManagerThread extends Thread {
	 ...
	 protected String torontoServer2Name = "";
	 ...
	 public ReplicaManagerThread(DatagramSocket rmSocket,DatagramPacket rcvPacket,byte[] rcvData,String tServer2Name, boolean flag)
	 {
		 this.rmSock = rmSocket;
		 this.receivePacketFromRM = rcvPacket;
		 this.receiveData = rcvData;
		 this.torontoServer2Name = tServer2Name;
		 this.isNew = flag;
	 }
    public void run()
    {       
       if(isNew)
		...
		{					
			try
			{
			      ...
			      InetAddress IPAddress = InetAddress.getByName(torontoServer2Name);
			      ...
			      IPAddress = InetAddress.getByName(torontoServer2Name);
			      sendData = new byte[1024];
			      receiveData = new byte[1024];		
			      sendVal = "getItemLockDetails";
			      sendData = sendVal.getBytes();
			      sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 5002);
       ...
       System.out.println("Toronto Server 1 ready...");	
       ...
       mcThread = new MultiCastThread(2000,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(4002); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
