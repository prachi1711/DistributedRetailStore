...
public class ReplicaManagerThread extends Thread {
	 ...
	 protected String vancouverServer2Name = "";
	 ...
	 public ReplicaManagerThread(DatagramSocket rmSocket,DatagramPacket rcvPacket,byte[] rcvData,String vServer2Name, boolean flag)
	 {
		 this.rmSock = rmSocket;
		 this.receivePacketFromRM = rcvPacket;
		 this.receiveData = rcvData;
		 this.vancouverServer2Name = vServer2Name;
		 this.isNew = flag;
	 }
    public void run()
    {       
       if(isNew)
		...
		{					
			try
			{
			      ...
			      InetAddress IPAddress = InetAddress.getByName(vancouverServer2Name);
			      ...
			      IPAddress = InetAddress.getByName(vancouverServer2Name);
       ...
       System.out.println("Vancouver Server 1 ready...");	
       ...
       mcThread = new MultiCastThread(3000,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(4003); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
