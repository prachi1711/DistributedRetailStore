...
public class ReplicaManagerThread extends Thread {
    ...
    {       
       ...
       System.out.println("Vancouver Server 2 ready...");	
       ...
       mcThread = new MultiCastThread(3001,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(5003); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
