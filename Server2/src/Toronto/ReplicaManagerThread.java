...
public class ReplicaManagerThread extends Thread {
	 ...
	 protected String torontoServer1Name = "";
	 ...
	 public ReplicaManagerThread(DatagramSocket rmSocket,DatagramPacket rcvPacket,byte[] rcvData,String tServer1Name, boolean flag)
	 {
		 this.rmSock = rmSocket;
		 this.receivePacketFromRM = rcvPacket;
		 this.receiveData = rcvData;
		 this.torontoServer1Name = tServer1Name;
		 this.isNew = flag;
	 }
    public void run()
    {       
       if(isNew)
		...
		{					
			try
			{
			      ...
			      InetAddress IPAddress = InetAddress.getByName(torontoServer1Name);
			      ...
			      IPAddress = InetAddress.getByName(torontoServer1Name);
			      sendData = new byte[1024];
			      receiveData = new byte[1024];		
			      sendVal = "getItemLockDetails";
			      sendData = sendVal.getBytes();
			      sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 4002);
       ...
       TorontoServerImpl m = new TorontoServerImpl(itemHT,semaphoreHM);
       System.out.println("Toronto Server 2 ready...");	
       int multiCastPortNo = 4447;
       InetAddress multiCastAddress = null;
       try 
       {
    	   multiCastAddress = InetAddress.getByName("228.5.6.6");
       } 
       catch (UnknownHostException e1) 
       {		
    	   e1.printStackTrace();
       }
       mcThread = new MultiCastThread(2001,multiCastPortNo,multiCastAddress,m);
	   mcThread.start();
       try
       {
    	 serverSocket = new DatagramSocket(5002); // create a socket to bind with other servers using UDP     	      	  
    	 receiveData = new byte[1024];
         while(true)
       	 {            		
           	 DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
