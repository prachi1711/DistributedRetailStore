...
public class TorontoServerImpl implements RetailStoreOperations {
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(semaphoreHM.containsKey(itemID))
					{
							...
							if(i1.tryAcquire(500, TimeUnit.MILLISECONDS)) // locking the item to maintain concurrency
							{
							   ...
							   try
							   {
									...
									else if(flag == 2)//item available in the current store but is less than required quantity
									{
										...
										islockedV = intimateOtherServerRegLock(itemID,0,5003);
											...
											serverItemMap = checkStockFromOtherServers(itemID,serverItemMap);
								             ...
								             if(totalItemsAvailable >= noOfReqItem)
								             {
								            	 ...
								            	 else
								            	 {
								            		 ...
								            		 orderItemFromOtherStore(itemID,itemsAvailableWithMServer,5001);
								            		 orderItemFromOtherStore(itemID,noOfReqItem,5003);
								            	 }
								            	 buyStatus = true;
									         }
								             else
									...
									else if(flag == 3) // item not available in the current store
									{
										...
										if(islockedM && islockedV)							
										{
							             ...
							             if(totalItemsAvailable >= numberOfItem)
							             {
							            	 ...
							            	 {
							            		 orderItemFromOtherStore(itemID,numberOfItem,5001);
							            	 }
							            	 else
							            	 {
							            		 int tempNo = 0;
							            		 ...
							            		 orderItemFromOtherStore(itemID,itemsAvailableWithMServer,5001);
							            		 orderItemFromOtherStore(itemID,tempNo,5003);
							            	 }
							            	 buyStatus = true;
							             }
							             else
						...
						finally
						{
							...
							if(islockedV)
							{
								intimateOtherServerRegLock(itemID, 1, 5003);
							}
						}
					}
					else
				...
				if(c1.tryAcquire(500, TimeUnit.MILLISECONDS))
				{
					...
					if(inputFile.exists())
					{
								    			...
								    			try
												   {
														...
														else if(flag == 2)//item available in the current store but is less than required quantity
														{
															...
															islockedV = intimateOtherServerRegLock(desiredItemID,0,5003);
															if(islockedM && islockedV)							
															{
																Hashtable<String ,HashMap<Integer,Integer>> serverItemMap = new Hashtable<String ,HashMap<Integer,Integer>>();;
																...
																serverItemMap = checkStockFromOtherServers(desiredItemID,serverItemMap);
													             ...
													             if(totalItemsAvailable >= noOfReqItem)
													             {
													            	 ...
													            	 {
													            		 noOfReqItem = noOfReqItem - itemsAvailableWithMServer;
													            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithMServer,5001);												            				 												            			
													            		 orderItemFromOtherStore(desiredItemID,noOfReqItem,5003);
													            	 }
													            	 buyStatus = true;
														         }
													             else
														...
														else if(flag == 3) // item not available in the current store
														{
															...
															if(islockedM && islockedV)							
															{
												             ...
												             if(totalItemsAvailable >= desiredNumber)
												             {
												            	 ...
												            	 {
												            		 orderItemFromOtherStore(desiredItemID,desiredNumber,5001);
												            	 }
												            	 else
												            	 {
												            		 int tempNo = 0;
												            		 ...
												            		 orderItemFromOtherStore(desiredItemID,itemsAvailableWithMServer,5001);
												            		 orderItemFromOtherStore(desiredItemID,tempNo,5003);
												            	 }
												            	 buyStatus = true;
												             }
												             else
						...
						finally
						{
							...
							if(islockedV)
							{
								intimateOtherServerRegLock(desiredItemID, 1, 5003);
							}
						}
		 ...
		 */
			...
			public Hashtable<String ,HashMap<Integer,Integer>> checkStockFromOtherServers(int itemID,Hashtable<String ,HashMap<Integer,Integer>> serverItemMap) 
			{
				...
				try
				{
				      ...
				      sendData = sendVal.getBytes();
				      sendPacket = new DatagramPacket(sendData, sendData.length, IPAddress, 5003);
	...
	public static void main(String[] args) 
	{
		try
	    {  						
			DatagramSocket rmSocket = new DatagramSocket(5006); // create a socket to bind with replica Manager using UDP
			byte[] receiveData = new byte[1024];	
